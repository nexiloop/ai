...
export function useCodeHatChatUtils({
  isAuthenticated,
  chatId,
  messages,
  input,
  ...
  const ensureChatExists = async (userId: string) => {
    if (!isAuthenticated) {
      const storedGuestChatId = localStorage.getItem("guestCodehatChatId")
    ...
    if (messages.length === 0) {
      try {
        ...
        if (isAuthenticated) {
          // Use CodeHat-specific routing
          window.history.pushState(null, "", `/codehat/${newChat.id}`)
        } else {
          localStorage.setItem("guestCodehatChatId", newChat.id)
        }
