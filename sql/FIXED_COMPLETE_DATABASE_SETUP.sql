-- FIXED COMPLETE DATABASE SETUP FOR NEXILOOP AI AGENT SYSTEM
-- This file contains the complete database schema without problematic extensions
-- Run this entire file in Supabase SQL Editor to set up everything

-- Enable required extensions (only uuid-ossp which is standard in Supabase)
...
CREATE TABLE IF NOT EXISTS public.chats (
    id TEXT PRIMARY KEY DEFAULT 'chat_' || substr(md5(random()::text), 1, 10),
...
-- Add constraints safely (with proper DO blocks)
DO $$ 
BEGIN
...
BEGIN
    ...
    -- Agents policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Anyone can view public agents') THEN
        CREATE POLICY "Anyone can view public agents" ON public.agents
            FOR SELECT USING (is_public = true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own agents') THEN
        CREATE POLICY "Users can view own agents" ON public.agents
            FOR SELECT USING (auth.uid() = creator_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create agents') THEN
        CREATE POLICY "Users can create agents" ON public.agents
            FOR INSERT WITH CHECK (auth.uid() = creator_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own agents') THEN
        CREATE POLICY "Users can update own agents" ON public.agents
            FOR UPDATE USING (auth.uid() = creator_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own agents') THEN
        CREATE POLICY "Users can delete own agents" ON public.agents
            FOR DELETE USING (auth.uid() = creator_id);
    END IF;

    -- Chats policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own chats') THEN
        CREATE POLICY "Users can view own chats" ON public.chats
            FOR SELECT USING (auth.uid() = user_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create own chats') THEN
        CREATE POLICY "Users can create own chats" ON public.chats
            FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own chats') THEN
        CREATE POLICY "Users can update own chats" ON public.chats
            FOR UPDATE USING (auth.uid() = user_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete own chats') THEN
        CREATE POLICY "Users can delete own chats" ON public.chats
            FOR DELETE USING (auth.uid() = user_id);
    END IF;

    -- Messages policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view messages from own chats') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create messages in own chats') THEN
...
-- Add triggers to automatically update updated_at (safely)
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE 'No problematic extensions used - compatible with all PostgreSQL versions';
END $$;
